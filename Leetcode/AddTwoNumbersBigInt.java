 ...
 */
 ...
 public class AddTwoNumbersBigInt {
     ...
     public static ListNode addTwoNumbers(ListNode l1, ListNode l2) {
         ...
         BigInteger addToSum = BigInteger.ZERO; 
         // Combine into two numbers.
         for (int i = 0; i < no1.size(); i++) {
             addToSum = BigInteger.valueOf(no1.get(i));
             addToSum = addToSum.multiply(BigInteger.valueOf((long) Math.pow(10,i)));
 
             sum = sum.add(addToSum);
             System.out.println(i + " no1 add to: " + sum);
         }
 
         for (int j = 0; j < no2.size(); j++) {
             System.out.println("Add : " + no2.get(j) + " * " + Math.pow(10, j));
             addToSum = BigInteger.valueOf(no2.get(j));
             addToSum = addToSum.multiply(BigInteger.valueOf((long) Math.pow(10,j)));
